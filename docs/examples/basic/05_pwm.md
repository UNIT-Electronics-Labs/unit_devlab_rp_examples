...
int main() {
    ...
    // El slice tambien admite conteo simetrico (phase correct) y
    // polaridad de salida invertida; no son necesarios para atenuar un
    // LED con un barrido simple, por lo que se dejan en su valor por
    // defecto. Ejemplo de uso:
    // pwm_config_set_phase_correct(&config, true);
    // pwm_config_set_output_polarity(&config, true, true);
 
    pwm_init(slice, &config, true);  // true = arrancar de inmediato
 
    // El slice tambien puede generar una interrupcion al completar cada
    // periodo, alternativa a depender de sleep_ms() para actualizar el
    // nivel con mayor precision; no se emplea en esta practica:
...
`gpio_set_function(LED_PIN, GPIO_FUNC_PWM)` desconecta el pin del control GPIO simple empleado en Blink y lo asigna al periférico PWM. `pwm_gpio_to_slice_num()` y `pwm_gpio_to_channel()` obtienen el slice y canal que corresponden a `LED_PIN`, evitando calcularlos manualmente. `pwm_get_default_config()` produce una configuración base, sobre la cual `pwm_config_set_clkdiv()` fija la velocidad del contador y `pwm_config_set_wrap()` fija su periodo; conforme a la fórmula presentada en el Concepto Teórico, estos valores concretos (`clkdiv = 100`, `wrap = 255`) producen una frecuencia de PWM de aproximadamente 4.88 kHz, con una resolución de 256 niveles posibles (0 a 255) para el ciclo de trabajo —el propio barrido de `nivel` en el ciclo principal depende directamente de este valor de `wrap`—.
 
Las líneas comentadas documentan opciones disponibles en el periférico, pero no necesarias para atenuar un LED con un barrido simple. `pwm_config_set_phase_correct()` activaría el conteo simétrico descrito en el Concepto Teórico en lugar del conteo simple, ya suficiente aquí. `pwm_config_set_output_polarity()` invertiría la salida, haciendo que el LED brillara más con niveles bajos en lugar de altos. `pwm_init(slice, &config, true)` aplica la configuración y arranca el slice de inmediato. `pwm_set_irq_enabled()` habilitaría una interrupción al completar cada periodo, alternativa a depender de `sleep_ms()` para actualizar el nivel con mayor precisión; tampoco se emplea en esta práctica introductoria.
 
Dentro del ciclo principal, `pwm_set_chan_level()` actualiza el nivel de comparación del canal en cada iteración, mientras que `nivel` y `paso` implementan un barrido simple de ida y vuelta entre 0 y 255.
...
<div align="center">
  <img src="../resources/05_pwm.gif" width="300px" alt="LED integrado en un punto intermedio de atenuacion durante la practica de PWM">
