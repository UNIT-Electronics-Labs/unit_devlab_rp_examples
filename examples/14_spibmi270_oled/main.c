...
int main() {
    ...
    while (1) {
        ...
        // Despliegue en OLED: titulo grande arriba, y abajo dos columnas
        // (izquierda acelerometro, derecha giroscopio) con X/Y/Z cada una.
        // Fuente base 5x8: avance de 6*escala px por caracter.
        const int COL_IZQ = 0;
        const int COL_DER = 66;
        ssd1306_clear(&display);
        ssd1306_draw_string(&display, 4, 0, 2, "BMI270 SPI");
        ssd1306_draw_line(&display, 0, 18, 127, 18);
        ssd1306_draw_string(&display, COL_IZQ, 20, 1, "Acel[g]");
        ssd1306_draw_string(&display, COL_DER, 20, 1, "Giro[dps]");

        snprintf(linea, sizeof(linea), "X%+.2f", ax);
        ssd1306_draw_string(&display, COL_IZQ, 30, 1, linea);
        snprintf(linea, sizeof(linea), "X%+.1f", gx);
        ssd1306_draw_string(&display, COL_DER, 30, 1, linea);

        snprintf(linea, sizeof(linea), "Y%+.2f", ay);
        ssd1306_draw_string(&display, COL_IZQ, 40, 1, linea);
        snprintf(linea, sizeof(linea), "Y%+.1f", gy);
        ssd1306_draw_string(&display, COL_DER, 40, 1, linea);

        snprintf(linea, sizeof(linea), "Z%+.2f", az);
        ssd1306_draw_string(&display, COL_IZQ, 50, 1, linea);
        snprintf(linea, sizeof(linea), "Z%+.1f", gz);
        ssd1306_draw_string(&display, COL_DER, 50, 1, linea);
        ssd1306_show(&display);
